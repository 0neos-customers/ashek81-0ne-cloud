 ...
 * - OPTIONS(req)        — preflight handler
 * - AuthResult          — return type of validateExtensionAuth
 * - validateExtensionAuth(request) — Better Auth session + API key dual auth
 * - validateExtensionApiKey(request) — API key only auth
 *
 * Migrated from Clerk to Better Auth in Phase A.
 */
...
// =============================================
// Dual Auth: Better Auth session + API key
// =============================================
...
export async function validateExtensionAuth(request: NextRequest): Promise<AuthResult> {
  ...
  // 1) Try Better Auth session cookie first (browser/extension popups
  //    sharing cookies with the same site).
  try {
    const session = await auth.api.getSession({ headers: request.headers })
    if (session?.user?.id) {
      return { valid: true, authType: 'session', userId: session.user.id }
    }
  } catch {
    // fall through to bearer/API key path
  }

  // 2) Bearer API key (extension popup outside the browser session)
  if (authHeader) {
  ...
  return { valid: false, authType: null, error: 'Authentication required' }
}
